...
class Economy(Cog, name="Economy"):
    ...
    @command(name="deposit", description="Deposit all your PikaCoins from your wallet into your Bank!", aliases=["dep"])
    @cooldown(rate=1, per=2)
    async def deposit(self, ctx: Context, amount: int):
        ...
        if amount < 0:
            ...
            await error(ctx, em, "You can't deposit that amount!")
            return
        with open("./src/data/economy.json", "r") as f:
            data = json.load(f)
        ...
        if wallet < amount:
            em.colour = discord.Colour.red()
            await error(ctx, em, "Your wallet isn't that big! Try depositing a smaller amount.")
            return
        elif wallet >= amount:
