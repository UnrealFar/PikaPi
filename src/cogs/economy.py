...
class Economy(Cog, name="Economy"):
    ...
    async def balance(self, ctx: Context):
        ...
        em.add_field(name="Wallet", value=str(data[user_id]["wallet"]))
        em.add_field(name="Bank", value=str(data[user_id]["bank"]))
    ...
    async def deposit(self, ctx: Context, amount: int):
        ...
        em = discord.Embed()
        em.set_author(name=ctx.author.name, icon_url=ctx.author.avatar_url)
        em.description = f"You just deposited {amount} PikaCoins into your Bank! Your Bank balance is now: `{bank + wallet}`!"
        em.colour = discord.Colour.green()
